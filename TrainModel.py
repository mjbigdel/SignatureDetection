...
Result_GenForg = np.concatenate((np.ones(len(y_Gen)), np.zeros(len(y_Forg))), axis=0) # category GenOrForg
...
LamdaWeight = 0.01    
lossWeights = {"categoryUser": 1-LamdaWeight, "categoryGenOrForg": LamdaWeight}     
lr = 0.001
H=[]
...
save_model(model,'model_3_iter.h5')

#from keras.models import load_model
#model = load_model('model_2_iter.h5')
...
clfUser_SVM = SVC(kernel = 'rbf', C = 100, decision_function_shape='ovo',random_state=0, gamma='scale')
...
clfGenForg_SVM = SVC(kernel = 'rbf', random_state=0, gamma='scale', C = 100000000)
...
#plt.show()
plt.tight_layout()
plt.savefig("confusionmatrixGenForg.png")
plt.close()
...
#plt.show()
plt.tight_layout()
plt.savefig("confusionmatrixUser.png")
plt.close()
